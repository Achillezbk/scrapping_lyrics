...
def extract_lyrics(url: str) -> str:
    """Description: permet d'extraire les paroles dans une variable contenant le texte brute.

    Args:
        url: (str) URL ciblée.

    Returns: (str) Chaîne de caractères vide si la requête o.k, chaîne de caractères si la requête k.o
...
def get_clean_lyrics(raw_string: str, word_length: int = 2) -> list[str]:
    ...
    Returns: (list[str]) Mots en miniscule, nettoyés.
...
def remove_words(original_word: list[str], word_to_remove: list[str]) -> list[str]:
    ...
    Returns: (list[str]) Liste de mots sans les mots définis comme banals.
...
def get_all_clean_lyrics() -> list[str]:
    ...
    Returns: (list[str]) Tous les mots de toutes les chansons.

    """
    urls = get_all_urls()
    all_clean_lyrics = []
    for url in urls[:10]:  # limite de 10 url pour tester le script.
